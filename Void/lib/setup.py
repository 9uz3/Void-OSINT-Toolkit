...
for _key in C.THEME_ORDER:
    if _key == "rainbow":
        COLOR_OPTIONS.append(("rainbow", "RAINBOW", "#FF00FF", C.THEME_LABELS["rainbow"][0], C.THEME_LABELS["rainbow"][1]))
    else:
        _theme = C.THEMES[_key]
        _lbl = C.THEME_LABELS.get(_key, (_key, _key))
        COLOR_OPTIONS.append((_key, _key.upper(), _theme["neon"], _lbl[0], _lbl[1]))
...
def _draw(step, fr, sel_lang, sel_color, name_buf, sel_skip):
    ...
    head = Panel(
        Align.center(Text.from_markup(
            f"[bold {C.C_GOLD}]VOID OSINT CONFIG[/]  [{C.C_DIM}]·[/]  [{C.C_SILVER}]{_t('sub', fr)}[/]\n"
    ...
    if step == 1:
        ...
        t = C.THEMES.get(key, C.THEMES["cyan"])
        preview = Panel(
            Align.center(Text.from_markup(
                f"[{t['neon']}]████████[/]\n[{t['blood']}]████████[/]\n[bold {t['bright']}]{key.upper()}[/]"
            )),
            border_style=t["neon"], box=box.ROUNDED, padding=(1, 2),
        )
        grid.add_row(
        ...
        grid.add_row(
            Panel(
                Align.center(Text.from_markup(
                    f"[bold {C.C_WHITE}]{_t('name', fr)}[/]\n\n[bold {C.C_NEON}]{name_buf or 'Operator'}▌[/]"
                )),
        ...
        skip_txt = (
            ...
            f"{'[bold #88FFAA]►[/] ' if sel_skip else '   '}{_t('yes', fr)}\n"
            f"{'[bold #88FFAA]►[/] ' if not sel_skip else '   '}{_t('no', fr)}"
        )
        grid.add_row(
            Panel(Align.center(Text.from_markup(skip_txt)),
    ...
    foot = Panel(Align.center(Text.from_markup(f"[{C.C_DIM}]{_t('hint1' if step == 1 else 'hint2' if step == 2 else 'hint3' if step == 3 else 'hint4', fr)}[/]")),
...
def _outro(fr, s):
    ...
    console.print(Panel(
        ...
        title=f"[bold {C.C_GOLD}]VOID OSINT TOOLKIT[/]",
...
def run_setup_wizard(force=False):
    ...
    if not setup_required(force):
        C.apply_theme(C._THEME_ALIASES.get(s.get("theme", "white"), s.get("theme", "white")))
        return

    cls()
    console.print(Panel(Align.center(Text.from_markup(
        f"[bold {C.C_GOLD}]VOID OSINT CONFIG[/]\n[{C.C_DIM}]Initialisation…[/]"
    )), border_style=C.C_BLOOD))
    time.sleep(0.35)

    sel_lang = 0 if s.lang != "en" else 1
    fr = sel_lang == 0
    theme_key = C._THEME_ALIASES.get(s.get("theme", "white"), s.get("theme", "white"))
    ...
    # Suppress echo during wizard (cross-platform)
    _raw_old = None
    if os.name != "nt" and sys.stdin.isatty():
        import termios, tty
        fd = sys.stdin.fileno()
        _raw_old = termios.tcgetattr(fd)
    ...
    try:
        while step <= 4:
            ...
            elif step == 4 and key in (b"y", b"Y", b"o", b"O"):
                sel_skip = True
        ...
        if _raw_old is not None:
            import termios
            termios.tcsetattr(sys.stdin.fileno(), termios.TCSADRAIN, _raw_old)
